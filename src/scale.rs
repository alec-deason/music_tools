...
impl<C: PitchClassSpace> Scale<C> {
    ...
    pub fn is_step(&self, a: &PitchClassOctave<C>, b: &PitchClassOctave<C>) -> bool {
        ...
        let a_d = self
            .degree_from_note(&a.0);
        if let Some(a_d) = a_d {
            let a_1 = self.note_from_degree(a_d as i32+1);
            let new_a = PitchClassOctave(a_1.0, a.1);
            eprintln!("{} {}", a_1.0 == b.0, a_1.1 == b.1);
            &new_a == b
        } else {
            false
        }
...
mod tests {
    ...
    fn test_note_from_degree_positive_degrees() {
        ...
        let scale:Scale<PC> = Scale::new(PC::from_str("E").unwrap(), IntervalPattern::Major);
        assert_eq!(scale.note_from_degree(5), PitchClassOctave(PC::from_str("C♯").unwrap(), 0));
        assert_eq!(scale.note_from_degree(4), PitchClassOctave(PC::from_str("B").unwrap(), 0));
    }
    ...
    fn test_degree_from_note() {
        ...
        let scale:Scale<PC> = Scale::new(PC::from_str("E").unwrap(), IntervalPattern::Major);
        assert_eq!(scale.degree_from_note(&PC::from_str("C♯").unwrap()), Some(5));
        ...
        for tonic in &[PC::from_str("E").unwrap(), PC::from_str("C").unwrap(), PC::from_str("A").unwrap()] {
            for pattern in &[IntervalPattern::Major, IntervalPattern::Minor] {
                ...
                for i in 0..scale.len()*2 {
                    ...
                    eprintln!("{:?} {:?} {:?}", a.min(b), a.max(b), scale);
                    assert!(scale.is_step(&b, &a));
                }
            }
        }
    ...
    fn is_step_across_octaves() {
        let scale:Scale<PC> = Scale::new(PC::from_str("E").unwrap(), IntervalPattern::Major);
        let a = PitchClassOctave(PC::from_str("A").unwrap(), 0);
        let b = PitchClassOctave(PC::from_str("B").unwrap(), 1);
        assert!(!scale.is_step(&b, &a));
        let a = PitchClassOctave(PC::from_str("A").unwrap(), 0);
        let b = PitchClassOctave(PC::from_str("B").unwrap(), -1);
        assert!(!scale.is_step(&b, &a));
