...
mod tests {
    ...
    #[test]
    fn to_frequency() {
        let f = MIDIPitchSpace::to_frequency(&Semitone(60.0));
        assert_approx_eq!(f, 261.63, 0.01);
        let f = MIDIPitchSpace::to_frequency(&Semitone(69.0));
    ...
    fn to_pitch() {
        ...
        assert_eq!(MIDIPitchSpace::to_pitch(&o), Semitone(60.0));
        let o = PitchClassOctave(pc, 4);
        assert_eq!(MIDIPitchSpace::to_pitch(&o), Semitone(108.0));

        let pc = ChromaticPitchClassSpace::from_str("A").unwrap();
        let o = PitchClassOctave(pc, 0);
        assert_eq!(MIDIPitchSpace::to_pitch(&o), Semitone(69.0));
    }
}
